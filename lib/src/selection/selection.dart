...
class Selection {
  ...
  Selector _ensureSelector(Object selector) {
  ...
  Element _createNode(Element node, String name) {
    var ns = new NS.qualify(name);
    var space = ns.space == null ? node.namespaceUri : ns.space;
    return node.ownerDocument.createElementNS(space, ns.local);
  }
  ...
  String nodeAttr(String name) {
    var n = node;
    var ns = new NS.qualify(name);
    return ns.space == null ? node.getAttribute(ns.local) : node.getAttributeNS(ns.space, ns.local);
  }
  ...
  Selection insert(String name, Object beforeSelector) {
    return select((Element node, data, int i, int j) {
      var newNode = _createNode(node, name);
      var before = _ensureSelector(beforeSelector);
